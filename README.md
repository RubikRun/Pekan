...
A 2D game engine written in modern C++20 focused on modular architecture, ECS-driven design, and high-performance rendering.

## Overview

Pekan is a lightweight 2D game engine designed for modularity and performance.
Engine functionality is split into independent subsystems that can be enabled, replaced, or extended without modifying the core runtime,
making the engine suitable for both experimentation and larger structured projects.

**Key Features:**
- **Modular Subsystem Architecture**: Compile-time configurable subsystems with explicit dependencies and deterministic lifecycle management
...
- **Scene & Layer Management**: Scene abstraction combined with a layer stack for logic, rendering, and UI ordering
- **2D Batch Renderer**: Custom OpenGL-based renderer with automatic batching to reduce draw calls
- **Developer Tooling**: Built-in camera controllers, real-time ImGui integration, and an FPS widget
...
- Subsystem-based engine structure with deterministic initialization
- Layer stack for logic, rendering, UI, and debug ordering
...
- Demo of Pekan's support for rendering 2D shapes.
It features 60k shapes that all move around and change their transforms at random.
...
- Demo of Pekan's support for rendering 2D sprites.
It features 10k sprites that all move around and are gravitationally repelled by the mouse.
...
- Demo of Pekan's support for post-processing shaders.
It shows a few different types of post-processing shaders applied on a scene with many shapes.

https://github.com/user-attachments/assets/f9c37a2e-3a96-47ba-80c1-fca1c52a7dc5

- Demo of Pekan's support for transform hierarchies.
The character's arms, legs and head are parented by the body, and each limb is additionally parented by a joint
...
- Demo of Pekan's support for custom shaders and textures.
A few different shaders are shown, all of which combine 2 textures together based on some geometric animation
